...
public class GUIBuscarMarca extends JFrame {

	private String[] labels = { "Id", "Marca", "IdPais" };
	private JTextField[] texts = new JTextField[labels.length];
	
	public GUIBuscarMarca(){
		super();
		initGUI();
	}
	...
	private void initGUI() {
		...
		JPanel mainPanel = new JPanel(new BorderLayout(10, 10));
			...
			northPanel.add(new JLabel("Buscar Marca"));
				...
				button.addActionListener(new ActionListener(){
					...
					public void actionPerformed(ActionEvent e) {
						...
						if (create) {
							...
							try {
							Controlador.getInstance().accion(Events.BUSCAR_MARCA, Integer.parseInt(out[0]));
							} catch (Exception ex) {
	...
	public void update (TMarca marca){
		texts[0].setText("" + marca.getId());
		texts[1].setText(marca.getNombre());
		texts[2].setText("" + marca.getIdPais());
		for (int i = 0; i < labels.length; ++i){
			if (!labels[i].equalsIgnoreCase("Id"))
