...
public class GUIBuscarProducto extends JFrame {
	...
	private void initGUI() {
		...
		JPanel mainPanel = new JPanel(new BorderLayout(10, 10));
			...
			northPanel.add(new JLabel("Buscar Producto"));
				...
				button.addActionListener(new ActionListener(){
					...
					public void actionPerformed(ActionEvent e) {
						...
						if (create) {
							...
							try {
							Controlador.getInstance().accion(Events.BUSCAR_PRODUCTO, Integer.parseInt(out[0]));
							} catch (Exception ex) {
	...
	public void update (TProducto producto){
		texts[0].setText("" + producto.getId());
		texts[1].setText("" + producto.getIdMarca());
		texts[2].setText("" + producto.getNombre());
		texts[3].setText("" + producto.getPrecio());
		texts[4].setText("" + producto.getUnidades());
		for (int i = 0; i < labels.length; ++i){
			if (!labels[i].equalsIgnoreCase("Id"))
