...
public class GUIListarProducto extends JFrame {
	
	private JTable table;
	private String[] columnNames = { "#", "Id", "Nombre", "IdMarca", "Precio", "Unidades" }; 
		...
		JPanel mainPanel = new JPanel(new BorderLayout(10, 10));
			...
			button.addActionListener(new ActionListener(){
				...
				public void actionPerformed(ActionEvent e) {
					Controlador.getInstance().accion(Events.LISTAR_PRODUCTO, null);
					toFront();
				}
	...
	public void update (ArrayList<TProducto> res){
		 tableModel.setRowCount(0);
		 for (int i = 0; i < res.size(); i++)
			 tableModel.insertRow(i, new Object[] 
					 { i, res.get(i).getId(), res.get(i).getNombre(), res.get(i).getIdMarca(), res.get(i).getPrecio(), res.get(i).getUnidades() });
		 table.setModel(tableModel);
	}
